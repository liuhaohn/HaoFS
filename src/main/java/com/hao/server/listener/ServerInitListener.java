package com.hao.server.listener;
...
public class ServerInitListener implements ServletContextListener {
	...
	public void contextInitialized(final ServletContextEvent sce) {
		...
		// 2，校对文件块并清理临时文件夹
		Printer.instance.print("File system node information proofreading...");
		...
		if (fspf.isDirectory() && fspf.canRead() && fspf.canWrite()) {
			...
			fbu.checkFileBlocks();
			fbu.initTempDir();
			Printer.instance.print("Proofreading completed.");
		} else {
			Printer.instance.print("Error: failed to proofread the file system node information. The storage location cannot be read or written or does not exist.");
		}
	...
	public void contextDestroyed(final ServletContextEvent sce) {
		// 1，关闭动态监听
		NodeState localNode = fabricDao.getNodeByMspId(fabricDao.getLocalMspId());
		localNode.setAlive(false);
		fabricDao.insertNode(localNode);
		run = false;
		// 2，清理临时文件夹
		Printer.instance.print("Clean up temporary files...");
		fbu.initTempDir();
	}
