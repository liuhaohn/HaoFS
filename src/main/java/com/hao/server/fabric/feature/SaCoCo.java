...
import com.hao.server.fabric.feature.sacoco.PMasks;
 ...
 * <p>
 * The description method utilizes a weighted edge histogram where bins are populated on
 * the premise of whether the regions contain edges belonging to the salient contours,
 * while the discriminative power is further enhanced by integrating regional quantized
 * color information.
 *
 ...
 * @author Mathias Lux
 */
public class SaCoCo implements GlobalFeature {
    double[] feature;
...
//        meanImgS = meanImgS -(meanImgS / 10);
    ...
    public double getDistance(LireFeature f) {
        if (!(f instanceof SaCoCo)) throw new UnsupportedOperationException("Wrong descriptor.");
        return MetricsUtils.jsd(feature, ((SaCoCo) f).feature);
    }
