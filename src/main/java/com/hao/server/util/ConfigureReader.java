...
public class ConfigureReader {
	...
	private static ConfigureReader instance = new ConfigureReader();
	...
	private ConfigureReader() {
		...
		try {
			Printer.instance.print("Loading configuration file...");
			...
			initIPRules();
			initSignUpRules();
			Printer.instance.print("The configuration file is loaded. Checking configuration...");
			this.propertiesStatus = this.testServerPropertiesAndEffect();
			if (this.propertiesStatus == LEGAL_PROPERTIES) {
				Printer.instance.print("Ready.");
				startAccountRealTimeUpdateListener();
			}
		} catch (Exception e) {
			Printer.instance.print("Error: unable to load one or more configuration files（位于" + this.confdir + "路径下），Please try to delete the old configuration file and restart the app or check the permissions of the installation path（必须可读写）。");
		}
	}

	public static ConfigureReader instance() {
		return ConfigureReader.instance;
	}
	 ...
	 * </p>
	 * 
	 * @return java.util.List<ExtendStores> 所有扩展存储区对象的列表
	 */
	 ...
	 * <h2>获得验证码等级</h2>
	 * <p>
	 * 返回设置的验证码等级枚举类（VCLevel），包括：关闭（CLOSE）、简单（Simplified）、标准（Standard）
	 * </p>
	 * 
	 * @return VCLevel 验证码等级
	 */
		...
		if (ss != null) {
			...
			if (this.testServerPropertiesAndEffect() == 0) {
				try {
					...
					Printer.instance.print("Configuration update complete, ready.");
					return true;
				} catch (Exception e) {
					Printer.instance.print("Error: failed to update settings, unable to save to settings file.");
				}
			}
		}
		return false;
	...
	private int testServerPropertiesAndEffect() {
		Printer.instance.print("Checking server configuration...");
		final String pMustLogin = this.serverp.getProperty("mustLogin");
		if (pMustLogin == null) {
			Printer.instance.print("Warning: no login required configuration found, default (0) will be used.");
			this.mustLogin = "O";
		} else {
			if (!"N".equals(pMustLogin) && !"O".equals(pMustLogin)) {
				Printer.instance.print("Error: the configuration of the must login function is incorrect (it can only be set to \"O\" or \"N\"), please check again.");
		...
		if (ports == null) {
			Printer.instance.print("Warning: port configuration not found, default value (8080) will be used.");
			this.port = 8080;
		} else {
			try {
				this.port = Integer.parseInt(ports);
				if (this.port <= 0 || this.port > 65535) {
					Printer.instance.print("Error: the port number is not configured correctly, an integer between 1-65535 must be used.");
					return INVALID_PORT;
				}
			} catch (Exception e) {
				Printer.instance.print("Error: the port number is not configured correctly, an integer between 1-65535 must be used.");
		...
		if (logs == null) {
			Printer.instance.print("Warning: log level configuration not found, default value (E) will be used.");
			this.log = "E";
		} else {
			if (!logs.equals("N") && !logs.equals("R") && !logs.equals("E")) {
				Printer.instance.print("Error: the log level configuration is incorrect (can only be set to \"N\", \"R\" or \"E\"), please check again.");
				return INVALID_LOG;
			}
			this.log = logs;
		}
		...
		if (vcl == null) {
			Printer.instance.print("Warning: login verification code configuration not found, default value (standard) will be used.");
			...
			case "CLOSE":
				...
				break;
			default:
				Printer.instance.print("Error: login verification code is not configured correctly (can only be set to \"standard\", \"simp\" or \"close\"), please check again.");
				return INVALID_VC;
			}
		}
		// 是否允许用户修改密码
		...
		if (changePassword == null) {
			Printer.instance.print("Warning: no user modification password function configuration found, default value will be adopted (disabled).");
		...
		Printer.instance.print("Check it out.");
	...
	public void createDefaultServerPropertiesFile() {
		...
		try {
			dsp.store(new FileOutputStream(this.confdir + SERVER_PROPERTIES_FILE),
					"<This is the default hao server setting file. >");
	...
	private void createDefaultAccountPropertiesFile() {
		...
		try (FileOutputStream accountSettingOut = new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE)) {
			dap.store(accountSettingOut, "<This is the default hao account setting file. >");
		...
		if (cl == 0) { // folder对所有人公开
			...
			if (account != null) {
				if (cl == 1) { // folder对小组公开
					...
					String vGroup = accountp.getProperty(account + ".group"); // visitor组
					String cGroup = accountp.getProperty(f.getFolderCreator() + ".group"); //creator组
