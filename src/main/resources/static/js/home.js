...
// 开始登陆加载动画
function startLogin() {
    $("#loginpath").attr('disabled', 'disabled');
    $("#dologinButton").attr('disabled', 'disabled');
}

// 结束登陆加载动画
function finishLogin() {
    $("#loginpath").removeAttr('disabled');
...
function showAccountView(folderView) {
    ...
    $("#uploadFileButtonLi button").attr("onclick", "showUploadFileModel()");
    /*    $("#packageDownloadBox")
            .html(
...
function showFolderTable(folderView) {
    ...
    for (var i2 = folderView.fileList.length; i2 > 0; i2--) {
        ...
        $("#foldertable").append(createFileRow(fi, true, a, fi.requestAuthorization));
    }
}

// 下载，删除，授权
function createFileRow(fi, aL, aD, requestAuthorizationOrgs) {
    ...
    if (requestAuthorizationOrgs !== undefined) {
        fileRow = fileRow
            + "<button onclick='showResponseAuthorizationModel("
            + '"'
            + fi.fileId
            + '","'
            + requestAuthorizationOrgs
            + '"'
...
function showFileInfo(fileId) {
    $.ajax({
        url: 'homeController/getFileInfo.ajax',
        type: 'POST',
        data: {"fid": fileId},
        dataType: 'text',
        success: function (result) {
            var fileState = eval("(" + result + ")");
            $("#file_fileHash").text(fileState.fileHash);
            $("#file_fileName").text(fileState.fileName);
            $("#file_fileSize").text(fileState.fileSize);
            $("#file_time").text(fileState.time);
            $("#file_state").text(fileState.state);
            $("#file_featureHash").text(fileState.featureHash.substr(0,30)+"...");
            $("#file_organization").text(fileState.organization);
            $("#file_sliceOrganization").text(fileState.sliceOrganization.join(", "));
            $("#file_authorizedOrganization").text(fileState.authorizedOrganization.join(", "));
            $("#file_userOrganization").text(fileState.userOrganization.join(", "));
        }
    });
}
        ...
        xhr.onloadend = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                if (result === "uploadsuccess") {
                    ...
                    $("#uls_" + count).text("[失败]");
                } else if (result === "fileidentical") {
                    showUploadFileAlert("提示：存在与文件：[" + fname
                        + "]同hash的文件，可能是文件已存在，上传失败。");
                } else {
                    showUploadFileAlert("提示：出现意外错误，文件：[" + fname
                        + "]上传失败，上传被中断。");
                    $("#uls_" + count).text("[失败]");
...
/*--------文件校验模块start-------*/
var fs_check;  // 文件列表对象
function showCheckFileHashModel() {
    $("#uploadFileAlert-check").hide();
    $("#uploadFileAlert-check").text("");
    $("#uploadFileSuccess-check").hide();
    $("#uploadFileSuccess-check").text("");
    if (isUpLoading == false) {
        $("#filepath-check").removeAttr("disabled");
        $("#uploadfile-check").val("");
        $("#filepath-check").val("");
        $("#pros-check").width("0%");
        $("#pros-check").attr('aria-valuenow', '0');
        $("#umbutton-check").attr('disabled', false);
        $("#filecount-check").text("");
        $("#uploadstatus-check").html("");
        $("#selectcount-check").text("");
        $("#selectFileUpLoadModelAsAll-check").removeAttr("checked");
        $("#selectFileUpLoadModelAlert-check").hide();
    }
    $('#checkFileHashModal').modal('show');
}

// 点击文本框触发input:file选择文件动作
function checkpath_check() {
    $('#uploadfile-check').click();
}

// 获取选中文件
function getInputUpload_check() {
    fs_check = $("#uploadfile-check").get(0).files;
    showfilepath_check();
}

// 文件选中后自动回填文件路径
function showfilepath_check() {
    var filename = "";
    for (var i = 0; i < fs_check.length; i++) {
        filename = filename + fs_check[i].name;
        if (i < (fs_check.length - 1)) {
            filename = filename + "、";
        }
    }
    if (fs_check.length <= 1) {
        $("#selectcount-check").text("");
    } else {
        $("#selectcount-check").text("（共" + fs_check.length + "个）");
    }
    $("#filepath-check").val(filename);
}

// 检查文件是否能够上传
function checkUploadFile_check() {
    $("#uploadFileAlert-check").hide();
    $("#uploadFileAlert-check").text("");
    $("#uploadFileSuccess-check").hide();
    $("#uploadFileSuccess-check").text("");
    if (isUpLoading == false && isImporting == false) {
        if (fs_check != null && fs_check.length > 0) {
            $("#filepath-check").attr("disabled", "disabled");
            $("#umbutton-check").attr('disabled', true);
            isUpLoading = true;
            repeModelList = null;
            $("#uploadFileAlert-check").hide();
            $("#uploadFileAlert-check").text("");
            ...
            var maxFileIndex = 0;
            for (var i = 0; i < fs_check.length; i++) {
                filenames[i] = fs_check[i].name;
                if (fs_check[i].size > maxSize) {
                    maxSize = fs_check[i].size;
                    maxFileIndex = i;
                }
            }
            var namelist = JSON.stringify(filenames);
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result == "mustLogin") {
                        ...
                        switch (result) {
                            case "errorParameter":
                                showUploadFileAlert_check("提示：参数不正确，无法开始校验");
                                break;
                            default:
                                var resp = eval("(" + result + ")");
                                if (resp.checkResult == "fileTooLarge") {
                                    showUploadFileAlert_check("提示：文件["
                                        + resp.overSizeFile
                                        + "]的体积超过最大限制（"
                                        + resp.maxUploadFileSize
                                        + "），无法开始校验");
                                    ...
                                    showUploadFileAlert_check("提示：出现意外错误，无法开始校验");
                                }
                                break;
                        }
                    }
            ...
            showUploadFileAlert_check("提示：您未选择任何文件，无法开始校验");
        }
    } else {
        showUploadFileAlert_check("提示：另一项任务尚未完成，无法开始校验");
    }
}

// 执行文件上传并实现上传进度显示
function doupload_check(count) {
    var fcount = fs_check.length;
    $("#pros-check").width("0%");// 先将进度条置0
    $("#pros-check").attr('aria-valuenow', "0");
    var uploadfile = fs_check[count - 1];// 获取要上传的文件
    if (uploadfile != null) {
        var fname = uploadfile.name;
        if (fcount > 1) {
            $("#filecount").text("（" + count + "/" + fcount + "）");// 显示当前进度
        }
        $("#uploadstatus-check").prepend(
            "<p>" + html2Escape(fname) + "<span id='uls_" + count
            + "'>[正在校验...]</span></p>");
        xhr = new XMLHttpRequest();// 这东西类似于servlet里面的request
        ...
        fd.append("file", uploadfile);// 将文件对象添加到FormData对象中，字段名为uploadfile
        fd.append("fname", fname);
        xhr.open("POST", "homeController/doUploadCheckFile.ajax", true);// 上传目标

        xhr.upload.addEventListener("progress", uploadProgress_check, false);// 这个是对上传进度的监听
        // 上面的三个参数分别是：事件名（指定名称）、回调函数、是否冒泡（一般是false即可）
        ...
        xhr.onloadend = function () {
            ...
            if (xhr.status === 200) {
                var result = xhr.responseText;
                if (result == "uploadsuccess") {
                    $("#uploadFileSuccess-check").prepend(
                        "<p class='alert alert-success'>文件：\"" + html2Escape(fname) + "\" 存在，校验成功</p>");
                    $("#uls_" + count).text("[已校验完成]");
                } else if (result == 'filenotexist') {
                    $("#uploadFileSuccess-check").prepend(
                        "<p class='alert alert-danger'>文件：\"" + html2Escape(fname) + "\" 不存在，可能已经被篡改</p>");
                    $("#uls_" + count).text("[校验完成]");
                } else if (result == "filedeleted") {
                    $("#uploadFileSuccess-check").prepend(
                        "<p class='alert alert-warning'>文件：\"" + html2Escape(fname) + "\" 已经被删除</p>");
                    $("#uls_" + count).text("[校验完成]");
                } else {
                    showUploadFileAlert_check("提示：出现意外错误，文件：[" + fname
                        + "]校验失败，上传被中断。");
                    $("#uls_" + count).text("[校验失败]");
                }

                if (count < fcount) {
                    doupload_check(count + 1);
                } else {
                    if ($("#uploadFileSuccess-check").html() != "") {
                        showUploadFileSuccess();
                    }
                }
            } else {
                showUploadFileAlert_check("提示：出现意外错误，文件：[" + fname + "]校验失败，上传被中断。");
                $("#uls_" + count).text("[校验失败]");
            }
        };

    } else {
        showUploadFileAlert_check("提示：要上传的文件不存在。");
        $("#uploadstatus-check").prepend(
            "<p>未找到要校验的文件<span id='uls_" + count + "'>[校验失败]</span></p>");
    }
}

// 显示上传文件进度
function uploadProgress_check(evt) {
    if (evt.lengthComputable) {
        // evt.loaded：文件上传的大小 evt.total：文件总的大小
        var percentComplete = Math.round((evt.loaded) * 100 / evt.total);
        // 加载进度条，同时显示信息
        $("#pros-check").width(percentComplete + "%");
        $("#pros-check").attr('aria-valuenow', "" + percentComplete);
    }
}

// 显示上传文件错误提示
function showUploadFileAlert_check(txt) {
    isUpLoading = false;
    $("#filepath-check").removeAttr("disabled");
    $("#uploadFileAlert-check").show();
    $("#uploadFileAlert-check").text(txt);
    $("#umbutton-check").attr('disabled', false);
}

// 显示文件校验成功提示
function showUploadFileSuccess() {
    isUpLoading = false;
    $("#filepath-check").removeAttr("disabled");
    $("#uploadFileSuccess-check").show();
    $("#umbutton-check").attr('disabled', false);
}

// 取消上传文件
function abortUpload_check() {
    if (isUpLoading) {
        isUpLoading = false;
        if (xhr != null) {
            xhr.abort();
...
// 防止长耗时待机时会话超时的应答器，每分钟应答一次，防止掉线需要重新登录
function ping() {
    $.ajax({
